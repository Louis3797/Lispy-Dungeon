...
public class EscapeRoomValidator {
  ...
  private void validateNPCs(
      java.util.Map<String, NPC> npcs, java.util.Map<String, Room> rooms, ValidationResult result) {
    ...
    for (var entry : npcs.entrySet()) {
      ...
      // Validate location (optional - NPCs without location are templates for dynamic
      // spawning)
      if (npc.location != null && !npc.location.isEmpty()) {
        if (rooms != null && !rooms.containsKey(npc.location)) {
          result.addError(
              new ValidationError(
  ...
  private void validateQuizzes(
      java.util.Map<String, Quiz> quizzes,
