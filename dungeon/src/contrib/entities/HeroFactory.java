...
public final class HeroFactory {
  private static final Logger LOGGER = Logger.getLogger(HeroFactory.class.getName());
  ...
  private static final Consumer<Entity> EXECUTE_ACTIVE_HERO_SKILL = entity -> {
    LOGGER.info("Q key pressed! Attempting to execute active skill...");
    entity
        .fetch(SkillComponent.class)
        .ifPresent(
            skillComponent -> {
              LOGGER.info("SkillComponent found. Skills: " + skillComponent.getSkills().size());
              skillComponent
                  .activeSkill()
                  .ifPresent(
                      new Consumer<Skill>() {
                        ...
                        public void accept(Skill skill) {
                          LOGGER.info("Active skill found: " + skill.getClass().getSimpleName());
                          boolean executed = skill.execute(entity);
                          LOGGER.info("Skill execution result: " + executed);
                        }
                      });
              if (skillComponent.activeSkill().isEmpty()) {
                LOGGER.warning("No active skill found!");
              }
            });
  };

  private static Consumer<Entity> DEFAULT_DEATH = (hero) -> DialogUtils.showTextPopup(
      "You died!",
      "Game Over",
      () -> {
   ...
   *
   * <p>
   * The Entity is not added to the game yet.
   *
   * <p>
   * It will have a {@link CameraComponent}, {@link PlayerComponent}{,
   * {@link PlayerComponent},
   * {@link PositionComponent}, {@link VelocityComponent}, {@link DrawComponent},
   * {@link
  ...
  public static Entity newHero(CharacterClass characterClass, Consumer<Entity> deathCallback)
    ...
    Map<String, Animation> animationMap = Animation.loadAnimationSpritesheet(characterClass.textures());
    ...
    if (ENABLE_MOUSE_MOVEMENT) {
      ...
      inputComp.registerCallback(
          ...
          innerHero -> {
            ...
            Point mousePos = SkillTools.cursorPositionAsPoint().translate(Vector2.of(-0.5f, -0.25f));

            Point heroPos = innerHero
            ...
            // If the path is null or empty, try to find a nearby tile that is accessible
            // and
            // calculate a path to it
            if (path == null || path.getCount() == 0) {
              Tile nearTile = LevelUtils.tilesInRange(mousePos, 1f).stream()
   ...
   *
   * <p>
   * This will close the topmost UI dialog that has the close key configured to
   * close it.
   *
  ...
  public static void registerCloseUI(InputComponent ic) {
    ic.registerCallback(
        ...
        (e) -> {
          var firstUI = Game.levelEntities() // would be nice to directly access HudSystems
  ...
  private static void registerMovement(
    ...
    ic.registerCallback(
        ...
        entity -> {
          ...
          Vector2 updatedForce = existingForceOpt.map(existing -> existing.add(newForce)).orElse(newForce);
  ...
  private static void registerMouseLeftClick(InputComponent ic) {
    if (!Objects.equals(
      ...
      // If interact and skill are the same, only one callback can be used, so we only
      // interact if
  ...
  private static void resolveCollisionWithMomentum(Entity hero, Entity other) {
    ...
    if (optVc1.isEmpty() || optVc2.isEmpty())
      return;
  ...
  /**
   * Create a new Hero based on the given class with the default death callback
   * (restart level).
   *
