 ...
 *
 * <p>
 * CollisionSystem is a system which checks on execute whether the hit boxes of
 * two entities are
 * overlapping/colliding. In which case the corresponding Methods are called on
 * both entities.
 *
 * <p>
 * The system does imply the hit boxes are axis aligned.
 *
 * <p>
 * Each CollideComponent should only be informed when a collision begins or
 * ends. For this, a map
 * with all currently active collisions is stored and allows informing the
 * entities when a collision
 * ended.
 *
 * <p>
 * Entities with the {@link CollideComponent} will be processed by this system.
 */
public final class CollisionSystem extends System {
   ...
   *
   * <p>
   * The collision check will be performed only once for a given tuple of
   * entities, i.e. when
   ...
   *
   * <p>
   * Pair a given entity with every other entity with a higher ID.
   *
   ...
   *
   * <p>
   * This comparison is applied in the {@link #createDataPairs(Entity a)
   * createDataPairs} method
   * to create only tuples with entities with higher ID. This avoids performing a
   * collision check
   * twice for a pair of entities, first for (a,b) and second for (b,a).
   *
   * @param a First Entity.
   * @param b Second Entity
   * @return true when the comparison between a and b is less than zero, otherwise
   *         false.
   */
  ...
  /**
   * Create a pair of CollideComponents which is then used to check whether a
   * collision is happening
   * and to store in the internal map. Which allows informing the
   * CollideComponents about an ended
   * Collision.
   *
  ...
  private CollisionData newDataPair(final Entity a, final Entity b) {
    CollideComponent cca = a.fetch(CollideComponent.class)
        .orElseThrow(() -> MissingComponentException.build(a, CollideComponent.class));
    CollideComponent ccb = b.fetch(CollideComponent.class)
   ...
   *
   * <p>
   * Only allows a new collision to call the onEnter of the hitBoxes. An ongoing
   * collision is not
   * calling the onEnter of the hitBoxes. When a previous collision existed and no
   * longer is an
  ...
  private void onEnterLeaveCheck(final CollisionData cdata) {
    ...
    if (checkForCollision(cdata.ea, cdata.a, cdata.eb, cdata.b)) {
      ...
      Direction d = checkDirectionOfCollision(cdata.ea, cdata.a, cdata.eb, cdata.b);
      // a collision was happening and the two entities are no longer colliding, on
      // Leave
      // called once
  ...
  Direction checkDirectionOfCollision(
    ...
    // Take the distance between the center of both hitboxes in both X and Y
    // direction
    float dx = c1CenterX - c2CenterX;
    float dy = c1CenterY - c2CenterY;

    // Sum of the half widths and half heights = distance between the entities'
    // centers if they are
    // flush with each other
    float halfXWidths = c1Size.x() / 2f + c2Size.x() / 2f;
    float halfYHeights = c1Size.y() / 2f + c2Size.y() / 2f;

    // To get the overlap, we subtract the actual distance between the centers. If
    // the entities are
    // overlapping, this will be a positive number
    float overlapX = halfXWidths - Math.abs(dx);
    float overlapY = halfYHeights - Math.abs(dy);

    // Check which overlap is bigger to determine the axis of collision, then
    // determine the
  ...
  private void solidCollide(
    ...
    // Check if entity has VelocityComponent (items don't have it)
    var velocityOpt = eb.fetch(VelocityComponent.class);
    if (velocityOpt.isEmpty()) {
      // Entity doesn't have velocity (e.g., world items), can't move it
      return;
    }

    boolean bCanEnterOpenPits = velocityOpt.get().canEnterOpenPits();
    boolean bCanEnterWalls = velocityOpt.get().canEnterWalls();
    ...
    if (CollisionUtils.isCollidingWithLevel(
      ...
      // If we aren't in the first iteration, the other entity is also blocked, so
      // just don't do
      // anything
      return;
    }
